...
Why: Branch and PR workflows should validate the real production inputs without mutating shared akmods tags.
...
from ci_tools.main_check_candidate_akmods_cache import inspect_candidate_akmods_cache
from ci_tools.main_resolve_build_inputs import resolve_build_inputs, write_resolved_build_outputs
...
def main() -> None:
    # Normalize owner means: use lowercase so GHCR paths stay stable even if the
    # repository owner includes uppercase characters in GitHub's display text.
    image_org = normalize_owner(require_env("GITHUB_REPOSITORY_OWNER"))
    source_repo = require_env("AKMODS_REPO")

    # Reuse the same input-pinning logic as the main workflow so validation runs
    # do not silently test a different base image or kernel set.
